...
def dot_product_distance(x1: torch.Tensor, x2: torch.Tensor, eps=1e-8):
    """
    Calculate pairwise dot product distance between two tensors.
    """
    dot_product = torch.sum(x1 * x2, dim=1)
    return - dot_product # Distance property requires less == better, so we negate the dot product
